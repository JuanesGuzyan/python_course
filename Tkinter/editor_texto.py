...
class Editor(tkinter.Tk):
    def __init__(self):
        super().__init__()
        self.title('Enterprises - Editor de texto')
        ...
        self.campo_text = tkinter.Text(self, wrap=tkinter.WORD)
        #  Atributo de archivo
        self.archivo = None
    ...
    def _crear_componentes(self):
        frame_botones = tkinter.Frame(self, relief=tkinter.RAISED, bd=2)
        boton_abrir = tkinter.Button(frame_botones, text='Abrir', command=self._abrir_archivo)
        boton_guardar = tkinter.Button(frame_botones, text='Guardar', command=self._guardar)
        boton_guardar_como = tkinter.Button(frame_botones, text='Guardar como..', command=self._guardar_como)
        ...
        # Se coloco el frame de manera vertical
        ...
        self.campo_text.grid(row=0, column=1, sticky='NSWE')

    def _crear_menu(self):
        # Creamos el menu de la app
        menu_app = tkinter.Menu(self)
        self.config(menu=menu_app)
        # Agregamos las opciones a nuestro menu
        # Agregamos menu archivo
        menu_archivo = tkinter.Menu(menu_app, tearoff=False)
    ...
    def _abrir_archivo(self):
        ...
        self.campo_text.delete(1.0, tkinter.END)
        # Revisamos si hay un archivo
        if not self.archivo_abierto:
            return
        # Abrimos el archivo en modo r/w como un recurso
        with open(self.archivo_abierto.name, 'r+') as self.archivo:
            # leemos el contenido
            texto = self.archivo.read()
            # Insertamos el contenido del archivo en el campo de textp
            self.campo_text.insert(1.0, texto)
    ...
    def _guardar(self):
        ...
        if self.archivo_abierto:
            ...
            with open(self.archivo_abierto.name, 'w') as self.archivo:
                ...
                texto = self.campo_text.get(1.0, tkinter.END)
